...
namespace Shadowsocks.Std.Util.SystemProxy
{
    public static class Sysproxy
    {
        ...
        private static void Save()
        {
            ...
            {
                // TODO 待实现
                using StreamWriter sw = new StreamWriter(File.Open(Utils.GetTempPath(_userWininetConfigFile, null), FileMode.Create));
        ...
        private static void Read()
        {
            try
            {
                string configContent = File.ReadAllText(Utils.GetTempPath(_userWininetConfigFile, null));
